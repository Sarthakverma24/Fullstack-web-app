...
func CreateUserTable(session *gocql.Session) error {
	query := `
		CREATE TABLE IF NOT EXISTS User.User_data (
			user_id TEXT,
			user_name TEXT,
			phone_no TEXT,
			age DOUBLE,
...
func createKeyspace(session *gocql.Session) error {
	err := session.Query(`CREATE KEYSPACE IF NOT EXISTS User WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 3}`).Exec()
	if err != nil {
...
func NewScyllaDBSession() (Database, *gocql.Session) {
	...
	if err != nil {
		log.Fatalf("failed to create AVL table: %v", err)
	}
